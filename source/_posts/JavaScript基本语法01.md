...
* 3、布尔值(`Boolean`)：包括`true`和`false`
* 4、`Undefined`：当我们试图访问一个不存在的变量时，就会得到一个特殊的值`undefined`。除此之外，使用已定义却未赋值的变量也会如此，以为js会自动将变量在初始化之前的值设定为`undefined`。而`Undefined`类型的值只有一个，那就是`undefined`
